...
import { buildItunesSearchResponse, searchItunesSong } from "@/lib/itunes/search";
...
function getSearchMemoryStore(): SearchMemoryStore {
  const g = globalThis as typeof globalThis & {
    __SONGSHARE_ITUNES_SEARCH_MEMORY__?: SearchMemoryStore;
  };

  if (!g.__SONGSHARE_ITUNES_SEARCH_MEMORY__) {
    g.__SONGSHARE_ITUNES_SEARCH_MEMORY__ = {
  ...
  return g.__SONGSHARE_ITUNES_SEARCH_MEMORY__;
}
...
async function getCachedSearchItems(query: string, kind: SubjectKind): Promise<SearchItems> {
  ...
  const requestPromise = searchItunesSong({ query, kind });
...
export async function handleItunesSearchRequest(
  ...
  try {
    ...
    return NextResponse.json(
      {
        ...buildItunesSearchResponse({ query, kind, items: [] }),
        ok: false,
